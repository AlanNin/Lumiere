...
function SortAZModalButton({ onPress }: { onPress: () => void }) {
  return (
    <TouchableOpacity className="p-2" onPress={onPress}>
      <ArrowDownAZ color={colors.muted_foreground} size={20} strokeWidth={1.6} />
    </TouchableOpacity>
  );
}
...
export default function CategoriesScreen() {
  ...
  const { mutate: changeCategorySortOrder } = useMutation({
    mutationFn: ({ categoryId, newSortOrder }: { categoryId: number; newSortOrder: number }) =>
  ...
  return (
    <View className="flex-1 bg-background">
      <TabHeader
        ...
        scrollY={scrollY}
        customRightContent={
          <SortAZModalButton onPress={() => bottomDrawerAZRef.current?.present()} />
        }
      />
