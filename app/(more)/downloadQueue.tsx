...
export default function DownloadQueueScreen() {
  ...
  const [selectedQueueItem, setSelectedQueueItem] = useState<QueueDownloadItem | undefined>();
  ...
  function ToggleQueuePauseButton() {
    return (
      <TouchableOpacity className="p-2" onPress={toggleAllDownloadsPaused}>
        {areDownloadsPaused ? (
          <ArrowDownToLine color={colors.muted_foreground} size={20} strokeWidth={1.6} />
        ) : (
  ...
  function ToggleRemoveAllButton() {
    return (
      <TouchableOpacity className="p-2" onPress={() => clearQueueDrawerRef.current?.present()}>
  ...
  return (
    <View className="flex-1 bg-background">
      <TabHeader
        title={
          queueDownload.length > 0 ? `Download Queue (${queueDownload.length})` : 'Download Queue'
        }
        renderBackButton
        scrollY={scrollY}
