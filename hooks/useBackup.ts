...
class AppBackupManager {
  ...
  async createBackup(): Promise<void> {
    try {
      ...
      const novelCategories = await this.db.select().from(schemas.novelCategories);

      const imageFiles = await FileSystem.readDirectoryAsync(this.backgroundsDir);
  ...
  async restoreBackup(
    ...
    try {
      ...
      if (data.appConfig && typeof data.appConfig === 'object') {
        const safeConfigEntries: [string, string][] = Object.entries(data.appConfig)
      ...
      if (data.categories && Array.isArray(data.categories)) {
        for (const cat of data.categories) {
          if (strategy === 'skip') {
            await this.db.insert(schemas.categories).values(cat).onConflictDoNothing();
          } else {
      ...
      if (data.novels && Array.isArray(data.novels)) {
        for (const novel of data.novels) {
          if (strategy === 'skip') {
            await this.db.insert(schemas.novels).values(novel).onConflictDoNothing();
          } else {
      ...
      if (data.chapters && Array.isArray(data.chapters)) {
        for (const chapter of data.chapters) {
          if (strategy === 'skip') {
            await this.db.insert(schemas.novelChapters).values(chapter).onConflictDoNothing();
          } else {
      ...
      if (data.novelCategories && Array.isArray(data.novelCategories)) {
        for (const link of data.novelCategories) {
          if (strategy === 'skip') {
            await this.db.insert(schemas.novelCategories).values(link).onConflictDoNothing();
          } else {
